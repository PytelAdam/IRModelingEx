...
Bezpoœrednio z twierdzenia Girsanowa dostajemy, ¿e
...
terminow¹. Wyka¿, ¿e zmiany na krzywej terminowej dla ró¿nych tenorów s¹ równoleg³e (z dok³adnoœci¹ do znaku).
Czy to oznacza, ¿e zmiany s¹ perfekcyjnie liniowo skorelowane, tzn. korelacja Pearsona jest równa 1?

**Rozwi¹zanie:** WprowadŸmy oznaczenie $\tau := T-t$. Wtedy cenê obligacji zerokuponowej dla modelu ATS mo¿emy zapisaæ:
...
Cenê obligacji zerokuponowej $B$ mo¿emy wyraziæ przez stopy 
zerokuponowe $Y$:
...
Zatem zmiany w jednym czynniku stochastycznym s¹ równoleg³e, choæ niekoniecznie maj¹ ten sam znak, bo funkcja $n$ dla ró¿nych terminów zapadalnoœci mo¿e przyjmowaæ wartoœci dodatnie b¹dŸ ujemne.
...
SprawdŸmy nastêpnie korelacjê liniow¹. Nale¿y zatem sprawdziæ czy zmiany na krzywej zerokuponowej dla ró¿nych $\tau_1$ i $\tau_2$ s¹ ze sob¹ perfekcyjnie skorelowane.
Oznaczmy przez $Z(t,\Delta, T)$ zmianê w ci¹gu czasu $\Delta$ na tenorze $T=t+\tau$, tzn.
...
Sta³e te zale¿¹ od $t,\Delta,\tau_1, \tau_2$. Podstawiaj¹æ $Y$ do powy¿szego warunku mamy:
$$Y(t+\Delta,\tau_1) - a\,Y(t+\Delta,\tau_2) = Y(t,\tau_1) - a\,Y(t,\tau_2) + b, $$
co przy uwzglêdnieniu zale¿noœci pomiêdzy $Y$, a $n,m, r$ daje:
...
poprzednia równoœæ zachodzi je¿eli ka¿dy ze wspó³czynników jest równy $0$.
St¹d:
$$n(t+\Delta,\tau_1) = a\,n(t+\Delta,\tau_2),\quad n(t,\tau_1) = a\,n(t,\tau_2)$$
Zauwa¿my, ¿e je¿eli dla pewych $t,\tau_2\ge 0$ funkcja $n(t,\tau_2)=0$, to $n(t,\tau)=0$ dla wszystkich $\tau$. Poza tym przypadkiem, $a$ jest niezale¿ne od $\Delta$. Ustalmy $t$ takie, ¿e $n(t,\tau)\neq 0$. Z pierwszej równoœci dostajemy
$$\frac{n(t+\Delta,\tau_1)}{n(t,\tau_1)} = \frac{n(t+\Delta,\tau_2)}{n(t,\tau_2)},$$
a wiêc ilorazy s¹ niezale¿ne od drugiej zmiennej $\tau$ i zale¿¹ jedynie od $\Delta$  (oczywiœcie poza przypadkiem, gdy funkcja $n$ siê zeruje). Oznacza to, ¿e istniej¹ funkcje g³adkie $h, f$, ¿e dla ka¿dego $\tau, \Delta \ge0$ mamy:
$$ n(\Delta, \tau) = h(\Delta)\cdot f(\tau), \quad \forall\,\Delta\ge t,\ \tau\ge0$$

## Korelacje dla ATS cz.3 {.smaller}

Z g³adkoœci $n$ oraz argumentu gêstoœciowego wynika, ¿e funkcja $n$ jest postaci:
